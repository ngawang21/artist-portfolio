...
## 3b. Contact form — make messages actually reach the artist 📨
...
4. Test from the live site. On success the visitor sees a green toast with a **reference code**
   (`KDT-XXXXXX`); that same code is saved with the submission and included in the email, so the
   artist and the visitor can refer to the exact message. Errors show a red toast with the fallback
   address. 100% free, no third-party email provider needed.

> The repo has an unused **@netlify/plugin-emails** enabled in the Netlify UI (left over from the old
> e-commerce build). It is *not* used by the contact form and can be removed from the UI to avoid confusion.

## 3c. Confirmation emails to the visitor **and** the artist (Resend) ✉️

On top of the dashboard notification above, the repo ships a Netlify function
(`netlify/functions/submission-created.mjs`) that runs after every verified submission and sends
two emails carrying the same `KDT-…` reference:

- **to the visitor** — a confirmation in the language of the form they used (EN/FR), with the
  reference and a copy of their message; replying to it reaches the artist;
- **to the artist** — the full message; replying to it reaches the visitor directly.

The success toast tells the visitor a confirmation is on its way, so **set this up before the next
deploy**. It uses [Resend](https://resend.com) (free tier: 3,000 emails/month, 100/day — ample,
and the artist's Gmail can't be used as a sender directly, which is why a provider is needed).
One-time setup:

1. Create a free account at **resend.com** (sign up with GitHub or email).
2. **Resend → Domains → Add domain** → `kungadempatsang.com`. Resend shows 3–4 DNS records
   (DKIM + SPF). Add them at **Porkbun → DNS** exactly as shown, then click **Verify** in Resend
   (propagation can take up to an hour). Status must read **Verified**.
3. **Resend → API Keys → Create API key** (permission “Sending access” is enough). Copy the
   `re_…` key — it is shown only once.
4. Netlify → **Site configuration → Environment variables → Add a variable**:
   - `RESEND_API_KEY` = the `re_…` key *(required)*
   - `CONTACT_FROM` = e.g. `Kunga DEMPA TSANG <contact@kungadempatsang.com>` *(optional — this
     is the default; the address does not need a mailbox, only the verified domain)*
   - `CONTACT_ARTIST_EMAIL` = `dempatsang@gmail.com` *(optional — this is the default)*
5. Trigger a deploy, then **test from the live site**: send a message with your own email —
   you should get the green toast + reference, a confirmation in your inbox, and the artist copy
   at `dempatsang@gmail.com` (check spam the very first time).

If `RESEND_API_KEY` is missing the function quietly does nothing — the form still works and the
§3b dashboard notification still reaches the artist. Nothing can be lost: every submission is
always stored in Netlify → Forms.
...
- [ ] Contact form: `contact` + `contact-fr` show in Netlify → Forms; email notification to `dempatsang@gmail.com` set for both; tested live (green toast + reference code).
- [ ] Confirmation emails (§3c): domain verified in Resend, `RESEND_API_KEY` set in Netlify, and a live
      test message received by **both** the visitor address and `dempatsang@gmail.com`.
